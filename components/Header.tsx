...
const Header: React.FC = () => {
  ...
  return (
    <header className={`sticky top-0 z-30 bg-[#e0e0e0]/80 backdrop-blur-lg border-b border-black/10 shadow-sm transition-transform duration-300 ${visible ? 'translate-y-0' : '-translate-y-full'}`}>
        ...
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <nav className="hidden md:flex items-center gap-6">
            <div className="flex space-x-4">
              {navLinks.map((link) => (
                <a key={link.key} href={link.href} className="text-gray-700 hover:text-black px-3 py-2 rounded-md text-sm font-medium transition-colors">
                  {t(link.key)}
                </a>
              ))}
            </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-[#e0e0e0]/90 backdrop-blur-lg" id="mobile-menu">
