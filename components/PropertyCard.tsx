    ...
    <NeumorphicCard className="flex flex-col overflow-hidden transition-transform duration-300 hover:-translate-y-2">
      <div className="relative group">
        ...
        <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-10 transition-opacity duration-300 flex items-center justify-center">
          <img src={COMPANY_INFO.logoUrl} alt="Watermark" className="w-24 h-24 opacity-0 group-hover:opacity-50 transition-opacity duration-300" />
        </div>
        <div className="absolute top-3 left-3 px-3 py-1 bg-[#e0e0e0] rounded-full shadow-[3px_3px_6px_#bebebe,-3px_-3px_6px_#ffffff] text-xs font-semibold text-gray-700">
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <p className="text-2xl font-bold text-[#153B67] my-4">
          ...
          {property.es_negociable && <span className="text-xs text-gray-500 ml-2 font-normal">Negociable</span>}
        </p>
        ...
        <div className="mt-auto flex flex-col sm:flex-row gap-3">
          ...
          <a
            href={`${SOCIAL_LINKS.whatsapp1}?text=${encodeURIComponent(`Hola, me interesa la propiedad: ${property.titulo}`)}`}
            target="_blank"
